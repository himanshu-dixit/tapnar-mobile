...
$sql ="SELECT image_key,format,title from image,views,upvote,editor WHERE editor.image_id= image.image_id and image.image_id = views.image_id AND image.image_id = upvote.image_id and views.type ='daily' GROUP BY image.image_id Order by image.date desc limit ".$start." ,".$end;
...
if($result){ ?>
  <a href="http://<?php echo $_SERVER['SERVER_NAME'];?>/module/images/favorite.php?page=<?php echo $page+1;?>" class="next_page"></a>

<?php }

if(!$result){
  ?>
<center>No More Images</center>
  <?php
}
 ?>
